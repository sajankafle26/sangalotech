...
interface FormProps {
    initialData: Partial<StudentTestimonial>;
    isEditing?: boolean;
}

const StudentTestimonialForm: React.FC<FormProps> = ({ initialData, isEditing = false }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const url = isEditing ? `/api/testimonials/students/${item._id}` : '/api/testimonials/students';
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="name" className={labelClass}>Student Name</label>
                    <input type="text" name="name" id="name" value={item.name} onChange={handleChange} required className={inputClass} />
                </div>
                <div>
                    <label htmlFor="course" className={labelClass}>Course Taken</label>
                    <input type="text" name="course" id="course" value={item.course} onChange={handleChange} required className={inputClass} />
                </div>
            </div>
             <div>
                <ImageUpload 
