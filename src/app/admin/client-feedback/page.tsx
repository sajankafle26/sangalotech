...
const AdminClientFeedbackPage: React.FC = () => {
    ...
    const fetchTestimonials = async () => {
        setLoading(true);
        try {
            const res = await fetch('/api/testimonials/clients');
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const res = await fetch(`/api/testimonials/clients/${id}`, { method: 'DELETE' });
            if (!res.ok) {
                const data = await res.json();
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
            <div className="flex justify-between items-center mb-6">
                ...
                <Link href="/admin/client-feedback/new" className="px-4 py-2 font-bold text-white bg-red-500 rounded-md hover:bg-red-600 transition-colors">
                            ...
                            {testimonials.map((item) => (
                                <tr key={item._id?.toString()}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <Link href={`/admin/client-feedback/${item._id?.toString()}/edit`} className="text-indigo-600 hover:text-indigo-900 mr-4">Edit</Link>
...
export default AdminClientFeedbackPage;