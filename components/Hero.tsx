...
const HeroCourseCard: React.FC<{ course: Course }> = ({ course }) => (
  <div className="border-2 border-[#004A61] bg-white rounded-2xl shadow-lg p-4 overflow-hidden h-full">
    <div className="relative w-full h-40">
        <Image src={course.imageUrl} alt={course.title} fill style={{ objectFit: 'cover' }} className="rounded-lg" />
    </div>
    <div className="p-3">
      ...
      <div className="flex justify-between items-center my-3">
        ...
        <span className="text-xs bg-green-100 text-green-600 px-2 py-1 rounded font-medium">{course.duration}</span>
      </div>
    </div>
  </div>
);
...
const Hero: React.FC = () => {
    ...
    useEffect(() => {
        const fetchCourses = async () => {
            try {
                // Fix: Fetching courses for the hero slider dynamically.
        ...
        <div className="grid md:grid-cols-3 items-center gap-5 md:gap-10">
          <div className="flex flex-col justify-between md:gap-10 order-2 md:order-1 md:col-span-2">
            <div className="space-y-[20px] md:space-y-[24px]">
              ...
              <h1 className="text-[22px] md:text-[26px] lg:text-[30px] xl:text-[38px] leading-[110%]">
                <strong>
                  Become A <span className="font-bold text-[#00548B]">Job-Ready Full-Stack Software Developer</span> In Just 12 Weeks
                </strong>
              </h1>
              <p className="font-bold">...Even If You’ve Never written a single line of Code before</p>
              ...
              <div className="mt-6 mb-4 flex gap-2 text-sm lg:text-base">
                <Link href="/courses/js1" className="w-[50%] bg-[#00548B] hover:bg-[#003f66] flex items-center text-white gap-1 transition-all duration-300 rounded-md justify-center py-2 transform hover:-translate-y-1 shadow-md hover:shadow-lg">
